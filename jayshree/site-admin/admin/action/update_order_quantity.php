...
include ("../../../config.php");
  ...
  if (isset($_POST["updateOrderQuantity"])) {
    ...
    $sel_cart = $conn -> query("SELECT * FROM cart WHERE id = '$cart_id'");
    $row_cart = mysqli_fetch_assoc($sel_cart);
    $mrp = $row_cart['mrp'];
    $total_amt = $mrp * $qty;
    // $settle_status = 1;

    $sql_order = $conn -> query("SELECT * FROM orders WHERE cart_id = '$cart_id'");
    $row_order = mysqli_fetch_assoc($sql_order);
    $process_id = $row_order['process_id'];

    $sql_bookings = $conn -> query("SELECT * FROM bookings WHERE process_id = '$process_id'");
    $row_book = mysqli_fetch_assoc($sql_bookings);

    $book_amt = $row_book['amount'];

    $new_total_amt = $book_amt + $mrp;

    $update_book = $conn -> query("UPDATE bookings SET amount = '$new_total_amt' WHERE process_id = '$process_id' ");

    $sql = "UPDATE cart set quantity = '$qty', total_amount = '$total_amt' WHERE id = '$cart_id'";
    
    $run_query = mysqli_query($conn,$sql);

    if ($run_query && $new_total_amt) {
