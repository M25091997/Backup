...
<head>
  ...
  <title>ATTRIBUTE WISE SALES</title>
...
<?php 
$id = $_GET['id'];
$QQQ = $conn -> query("SELECT * FROM attributes WHERE id = '$id'");
$ID = $QQQ -> fetch_assoc();
$item_idx = $ID['item_id'];
$attr_n = $ID['name'];

$QQQ2 = $conn -> query("SELECT * FROM items WHERE id = '$item_idx'");
$ID2 = $QQQ2 -> fetch_assoc();
$item_namex = $ID2['name'];

?>
  <br>
  <div class="row"><center><h3>ATTRIBUTE WISE SALES : <?php echo $item_namex; ?> - <u> <?php echo $attr_n; ?></u></h3></center></div>
  <br>
