...
$sel_user = $conn -> query("SELECT * FROM accounts WHERE id = '$user_id' && account_type = '1' ");
$account_type = mysqli_fetch_assoc($sel_user)['account_type'];

// $pro_rating = 0;
if($account_type == 1){
  $QBanner = $conn -> query("SELECT * FROM corporate_category ORDER BY priority_no ASC");
...
$Q = $conn -> query("SELECT * FROM items WHERE corporate_id = '$super_cat_id' ORDER BY id DESC LIMIT 5 ");
$product = array();
  while ($itemData = mysqli_fetch_array($Q)) { 
        ...
        $catq = $conn -> query("SELECT * FROM corporate_category WHERE id = '$cat_id'");
