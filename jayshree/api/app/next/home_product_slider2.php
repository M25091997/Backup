...
$QBanner = $conn -> query("SELECT * FROM super_category WHERE id = '2'");
$bannerData = mysqli_fetch_array($QBanner);
$banner = $url.$bannerData['banner'];


$Q = $conn -> query("SELECT * FROM items WHERE category_id = '2' ORDER BY id DESC LIMIT 3 ");
  ...
  array_push($product, array( 'response' => 1, 'name' => substr($name, 0, 23)."...", 'hindi_name' => $hindi_name, 'id' => $item_id, 'cat_name' => $cat_name, 'image' => $img, 'price' => $price, 'attr_id' => $attribute_id, 'category_id' => '2', 'attribute' => $attribute_name));

  // $pro_rating = 0;

} 

$result = array('banner' => $banner, 'product' => $product, 'category_id' => '2');
