...
$account_id = $_POST['account_id'];
	...
	$remo_cart = $conn -> query("DELETE FROM cart WHERE quantity = '0'");
	// print_r(json_encode($remo_cart));
...
if (isset($account_id)) {
    ...
    //         	while ($msg_q = mysqli_fetch_array($aq)) {
            	...
            	$media_q = $conn -> query("SELECT * FROM media WHERE media_number = '$media_number'");
              	...
              	$total_pri = $it_price * $quantity;
				$total_price = round($total_pri, 2);
              	array_push($cart_items_array, array('id' => $cart_id, 'item_id' => $item_id, 'item_name' => $item_name, 'hindi_name' => $hindi_name, 'item_price' => $it_price, 'quantity' => $quantity, 'total_amount' => $total_price, 'image' => $product_img, 'mrp' => $mrp_price ));
