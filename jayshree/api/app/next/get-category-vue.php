...
$msg_query_T = $conn -> query("SELECT * FROM items WHERE category_id = '$id'");
...
$msg_query = $conn -> query("SELECT * FROM items WHERE category_id = '$category_id' ORDER BY id DESC");
...
$total_products = mysqli_num_rows($msg_query);
        ...
        while ($item_Data = mysqli_fetch_array($attr_q)) { 
              ...
              array_push($result, array('id' => $item_id, 'slug' => $item_slug, 'attributeId' => $attribute_id, 'cat_name' => $category_name, 'name' => $product_name, 'image' => $product_img, 'mrp' => $mrp, 'price' => $price, 'discount' => $off, 'attributeName' => $attribute_name, 'ratings' => $avg_rating, 'review' => $count_row_rating ));
