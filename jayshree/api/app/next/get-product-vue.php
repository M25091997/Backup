...
if ($id != "" ) {
		...
		$msg_query = $conn -> query("SELECT * FROM items WHERE slug = '$id'");	
        ...
        if ($count_row_rating != 0) {
              ...
              array_push($product, array('id' => $msg_q['id'], 'name' => $msg_q['name'], 'details' => $msg_q['details'], 'image' => $product_img, 'images' => $image_array, 'category_id' => $category_id,'cat_name' => $cat_name, 'price' => $attr_price, 'ratings' => $avg_rating, 'review' => $count_row_rating, 'slug' => $msg_q['slug']));
