...
if (isset($_POST['total_cart_value']) && isset($_POST['account_id'])) {
    ...
    // $user_sql = "SELECT * FROM accounts WHERE id='$account_id'";
    // $user_query = mysqli_query($conn, $user_sql);
    // $user_count = mysqli_num_rows($user_query);

    // if ($user_count > 0) {
    //     array('response' => 'Please remove/buy previous bundle!');
    // } else {
    ...
    if ($payment_status == "Credit") {
        ...
        try { #try 1
            ...
            $result = array('response' => '1', 'order-id' => "CTIND" . substr($process_id, 0, 8));
        } catch (PDOException $e) #try 1
        {
            echo $error_msg = "Error: " . $e->getMessage();
        }
        ...
        if ($subscription == "1") {
            $time = date('Y-m-d');
            // enter skip
            $bSql = "SELECT * FROM bookings WHERE account_id = '$account_id' AND subs_status= '1' AND subscription !='1' ORDER BY id DESC limit 1";
