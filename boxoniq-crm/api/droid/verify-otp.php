...
if ($account_id != '' && $otp != '') {
    ...
    if (mysqli_num_rows($account_arr) > 0) {
        ...
        $baby_name = $row_user['baby_name'];
        $baby_dob = $row_user['baby_dob'];

        $result = array('response' => '1', 'accountId' => $account_id, 'address' => $address, 'name' => $name, 'email' => $email, 'phone' => $phone, 'baby_name' => $baby_name, 'baby_dob' => $baby_dob);
    } else {
        $result = array('response' => '0', 'msg' => 'Something Went Wrong');
    }
} else {
    $result = array('response' => '0', 'msg' => 'Account id or otp wrong');
}
