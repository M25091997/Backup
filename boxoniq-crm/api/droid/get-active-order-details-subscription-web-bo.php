...
if (isset($process_id)) {
  ...
  if ($bk_count > 0) {
    ...
    while ($msg_q = mysqli_fetch_array($msg_query)) {
      ...
      array_push($orders_array, array('response' => '1','cat_id' => $cat_id, 'process_id' => $process_id, 'item_id' => $item_id, 'super_cat_name' => $super_cat_name, 'attr_id' => $atrid, 'item_attr' => $item_attr, 'attr_price' => $atr_price, 'img' => $img, 'item_name' => $item_name, 'quantity' => $quantity, 'amount' => $up_amt, 'cart_id' => $cart_id, 'attribute' => $attribute));
    }
