...
if ( isset($data['cart_id']) && isset($data['key']) && isset($data['qty']) && isset($data['mrp'])) {
    ...
    $select_cart = $conn -> query("SELECT quantity FROM cart WHERE id = '$cart_id'");
    $row_sel_cart = mysqli_fetch_assoc($select_cart);
    $qtyorg = $row_sel_cart['quantity'];
...
try { #try 1
    ...
    $stmt = $conn_pdo -> prepare("UPDATE cart SET quantity = :qty, total_amount = :total_amount WHERE id = :cart_id");
