...
$random_id = substr(rand().time(), 6);
		...
		$msg_query = $conn -> query("SELECT * FROM subs_order_history WHERE process_id = '$process_id' ORDER BY id DESC ");	
            while ($msg_q = mysqli_fetch_array($msg_query)) {
              ...
              array_push($orders_array, array('response' => '1', 'random' => $random_id, 'item_id' => $item_id, 'img' => $img, 'item_name' => $item_name, 'item_price' => $item_price, 'quantity' => $quantity, 'amount' => $total_amount, 'status' => $status));
