...
$Q = $conn -> query("SELECT * FROM items WHERE category_id = '18' ORDER BY id DESC LIMIT 20 ");
  ...
  array_push($result, array( 'name' => substr($name, 0, 23)."...", 'id' => $item_id, 'slug' => $item_slug, 'cat_name' => $cat_name, 'subcat_name' => $subcat_name,  'attributeId' => $attribute_id, 'image' => $img, 'mrp' => $mrp, 'price' => $price, 'discount' => $off, 'attribute-name' => $attribute_name, 'ratings' => $avg_rating, 'review' => $count_row_rating  ));
