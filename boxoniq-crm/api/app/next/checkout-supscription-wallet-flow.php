...
// check user sub 
$b_sql = "SELECT * FROM subs_booking_history WHERE user_id = '$account_id' AND subs_status = 1 ";
...
if ($b_count > 0) {
    ...
    if ($payment_status == "Credit") {
        ...
        try { #try 1
            ...
            // prepare sql and bind parameters
            $stmt_bookings = $conn_pdo->prepare("INSERT INTO bookings ( process_id, creation_time, creation_date, payment_mode, payment_status, account_id, amount, subscription, subscription_month, is_coupon, coupon_id, address_id ) VALUES ( :process_id, :creation_time, :creation_date, :payment_mode, :payment_status, :account_id, :amount, :subscription, :subscription_month, :is_coupon, :coupon_id, :address_id )");
        ...
        if ($last_entry_count > 0) {
            ...
            if (isset($last_entry_id)) {
                ...
                if ($bcount > 0) {
                    ...
                    $iSql_his = "INSERT INTO subs_booking_history (user_id,process_id,subsprocess_id,total,date,total_items,isskip,iscancel,is_coupon,coupon_id,address_id,payment_mode,payment_transaction_id,payment_status,picker_transaction_id) VALUES ('$user_id','$process_id','$subsprocess_id','$amount','$nxtdate','$ocount','$isskip','$isskip','$is_coupon','$coupon_id','$address_id','$payment_mode','$payment_transaction_id','$pyament_status','$picker_transaction_id')";
                    $iquery_his = mysqli_query($conn, $iSql_his);
                    ...
                    while ($order_row = mysqli_fetch_assoc($order_query)) {
                        ...
                        while ($cart_row = mysqli_fetch_assoc($cart_query)) {
                            ...
                            $cat_id = $cart_row['cat_id'];
                            $account_id = $cart_row['account_id'];
                            ...
                            $iSql = "INSERT INTO subs_cart (item_id,attr_id,attr_price,item_qty,delete_status,account_id,cat_id) VALUES ('$item_id','$attr_id','$attr_price','$attr_qty','0','$account_id','$cat_id')";
                            $iquery = mysqli_query($conn, $iSql);
                            $sub_cart_id = mysqli_insert_id($conn);

                            $iSql_history = "INSERT INTO subs_cart_history (item_id,attr_id,attr_price,item_qty,delete_status,account_id,cat_id) VALUES ('$item_id','$attr_id','$attr_price','$attr_qty','0','$account_id','$cat_id')";
                            $iquery_history = mysqli_query($conn, $iSql_history);
                            $sub_cart_id_history = mysqli_insert_id($conn);

                            // ==========================================
                            // subs_order
                            $sub_order_sql = "INSERT INTO subs_order (process_id,subprocess_id,sub_cart_id,cat_id) VALUES('$process_id','$subsprocess_id','$sub_cart_id','$cat_id')";
                            $sub_order_query = mysqli_query($conn, $sub_order_sql);

                             // subs_order
                             $sub_order_sql_history = "INSERT INTO subs_order_history (process_id,subprocess_id,sub_cart_id,cat_id) VALUES('$process_id','$subsprocess_id','$sub_cart_id_history','$cat_id')";
