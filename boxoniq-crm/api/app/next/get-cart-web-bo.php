	  ...
	  $total_cart_value = 0;
        ...
        while($row_get_cart = mysqli_fetch_assoc($get_cart)){
            	...
            	$media_q = $conn -> query("SELECT * FROM cover_media WHERE media_number = '$media_number'");
              	...
              	array_push($cart_items_array, array('id' => $cart_id, 'item_id' => $item_id, 'item_name' => $item_name, 'item_desc' => $item_desc, 'item_price' => $it_price, 'quantity' => $quantity, 'total_amount' => $total_amount, 'image' => $product_img, 'mrp' => $mrp_price, 'attribute' => $it_name ));
...
if($count_cat >= $count_super_cat){
	$hi = 1;
	$bundal_discount = round(5/100*$total_cart_value);
}else{
	$hi=0;
	$bundal_discount = 0;
}
...
$total = array('sub_total' => $total_cart_value, 'bundle_discount' => $bundal_discount, 'coupon_discount' => $coupon_discount, 'del_charge' => $del_charge, 'grand_total' => $grand_total);

$result = array('cartItem' => $final_cart_array, 'total' => $total);
