...
$Q = $conn -> query("SELECT * FROM items WHERE category_id = '2' ORDER BY id DESC");
  ...
  while ($itemData = mysqli_fetch_array($Q)) { 
        ...
        $item_url = $site_url."/media/".$item_image;
  ...
  array_push($result, array( 'name' => substr($name, 0, 23)."...", 'id' => $item_id, 'cat_name' => $cat_name, 'attributeId' => $attribute_id, 'image' => $img));
