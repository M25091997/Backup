...
if (isset($account_id)) {
    ...
    //         	while ($msg_q = mysqli_fetch_array($aq)) {
            	...
            	$media_q = $conn -> query("SELECT * FROM media WHERE media_number = '$media_number'");
              	...
              	array_push($cart_items_array, array('id' => $cart_id, 'item_id' => $item_id, 'item_name' => $item_name, 'item_price' => $it_price, 'quantity' => $quantity, 'total_amount' => $total_amount, 'image' => $product_img, 'mrp' => $mrp_price, 'item_name' => $it_name ));
