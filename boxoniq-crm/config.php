...
function addToWallet($customer_id, $tran_id, $amount, $type){

    global $conn;
    global $conn_pdo, $dbHost,$dbName,$dbUsername,$dbPassword;


    try { #try 1
        
        $msg = "₹ ".$amount." Added for # ".$tran_id." in wallet";
        // $type = $type;            
    ...
    $select_wallet = $conn -> query("SELECT * FROM wallet WHERE user_id = '{$customer_id}'");
    $count = mysqli_num_rows($select_wallet);
    // print_r($count);
    // exit();

if (mysqli_num_rows($select_wallet) > 0) {
        $row_wallet = mysqli_fetch_array($select_wallet);
        $old_amount = $row_wallet['amount'];
        $old_wallet_id = $row_wallet['id'];
        $new_amount = $old_amount + $amount;
        $stmt = $conn_pdo -> prepare("UPDATE wallet SET amount = :amount, tran_id = :tran_id WHERE user_id = :customer_id ");
}
    ...
    $wallet_id = $conn_pdo->lastInsertId();
    if($wallet_id == 0){
    ...
    if ($execution_flag) {
        ...
        $stmt_his = $conn_pdo -> prepare("INSERT INTO wallet_history ( wallet_id, amount, tran_id, type, msg, created_on ) VALUES ( :wallet_id, :amount, :tran_id, :type, :msg, :trn_date )");

            $stmt_his->bindParam(':tran_id', $tran_id);
            $stmt_his->bindParam(':type', $type);
            $stmt_his->bindParam(':msg', $msg);

            $stmt_his->bindParam(':wallet_id', $wallet_id);
            $stmt_his->bindParam(':amount', $amount);
            $stmt_his->bindParam(':trn_date', $trn_date);

        

    $execution_flag_his = $stmt_his -> execute(); // Executing the query

        if($execution_flag_his) {
                $result = array( 'response' => 1, 'msg' => 'Successfully Updated amounts and rating....');
...
// // give total coin
function ReferCoins( $account_id, $coins_to_be_given){
    global $conn;
    $msg = "{$coins_to_be_given} coins Earned for Referring";
    $type = "IN";
    $created_at = date("Y-m-d H:i:s");
    ...
    // insert data into coin wallwt history 
    $sql2 = "insert into coin_wallet_history (account_id, msg, type, created_at, amt) values ('{$account_id}','{$msg}','{$type}','{$created_at}','{$coins_to_be_given}')";
    $query2 = mysqli_query($conn, $sql2);
