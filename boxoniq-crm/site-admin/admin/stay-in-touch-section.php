...
<head>
  ...
  <title>Stay In Touch Section</title>
...
<?php $msg_query = $conn -> query("SELECT * FROM stay_touch");  
...
while ($sData = mysqli_fetch_array($msg_query)) { 
  ...
  $get_user = $conn -> query("SELECT * FROM accounts WHERE id = '$user_id'");
  ...
  ?>
    <tr>
      ...
      <td>
        ...
        <button class="btn btn-danger delete_stay" stay_delete_id=<?php echo $id ?>><i class="fa fa-trash" aria-hidden="true"></i></button>
...
$(document).ready(function(){
    ...
    $(".delete_stay").click(function(){
        var stay_delete_id=$(this).attr('stay_delete_id');
        // alert(stay_delete_id);
        // return;
        $.ajax({
            url:"action/delete_stay.php",
            method:"POST",
            data:{deleteStay:1,del_id:stay_delete_id},
            dataType:"json",
