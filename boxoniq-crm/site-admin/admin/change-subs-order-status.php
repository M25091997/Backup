...
<?php  
    ...
    //  $cQ = $conn -> query("SELECT * FROM subs_bookings_history WHERE id = '$order_id' ORDER BY id DESC "); 
    //  while ($rowD = mysqli_fetch_array($cQ)) { 
    //     $account_id = $rowD['account_id'];
    //     $alloted_coins = $rowD['alloted_coins'];  
    // }

    // $cR = $conn -> query("SELECT * FROM referral WHERE reciever_id = '$account_id' AND active = 0"); 
    //  while ($rowDR = mysqli_fetch_array($cR)) { 
    //     $referrer_id = $rowDR['referrer_id'];
    //     $referrer_coin = $rowDR['referrer_coin'];  
    // }
    ...
    // _SEND_MESSAGE_NEW($mobile, "Dear Customer, Your order id #".$order_id." has been Delivered Successfully. Hope you are satisfied with our service, please rate us on the Play Store accordingly - CityIndia.in", "1207162169235854506"); 
    ...
    // GiveCoins( $account_id, $alloted_coins, $order_id);
    // ReferCoins( $referrer_id, $referrer_coin);


    $R5 = $conn->query("UPDATE subs_booking_history SET payment_status = '1' WHERE id = '$order_id' "); }
    ...
    $R4 = $conn->query("UPDATE subs_booking_history SET order_status = '$status_id' WHERE id = '$order_id' ");
    

    if ($R4) {  header('Location: all-subs-orders.php'); }
