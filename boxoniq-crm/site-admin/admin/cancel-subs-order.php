...
$by = $_GET['by'];
     ...
     $cQ = $conn -> query("SELECT * FROM subs_booking_history WHERE id = '$order_id' ORDER BY id DESC "); 
  ...
  //    while ($sD = mysqli_fetch_array($aQ)) { $mobile = $sD['phone']; }
     ...
     $R4 = $conn->query("UPDATE subs_booking_history SET order_status = '5' WHERE id = '$order_id' ");
    ...
    $cQOrder = $conn -> query("SELECT * FROM subs_order_history WHERE process_id = '$process_id' ORDER BY id DESC "); 
           while ($rowDOrders = mysqli_fetch_array($cQOrder)) {
               $cart_id = $rowDOrders['sub_cart_id'];

          $cQCart = $conn -> query("SELECT * FROM subs_cart_history WHERE id = '$cart_id' && checkout = '1' ORDER BY id DESC "); 
               $rowDCart = mysqli_fetch_array($cQCart);
               $attribute_id = $rowDCart['attr_id'];
               $quantity = $rowDCart['item_qty'];
	...
	// if ($by == "CUSTOMER") { header('Location: '.$site_url.'/account/my-orders.php'); } else { header('Location: all-orders.php'); }
     ...
     header('Location: all-subs-orders.php');
