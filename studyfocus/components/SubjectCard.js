...
export const SubjectCard = (props) => {
    ...
    return (
        <Link href={genLinkUrlSub(subject.subject_name)}><div className="col-xxl-4 col-xl-4 col-lg-4 col-md-6 col-sm-6">
            <div style={{cursor:'pointer'}} className="category__item mb-30 transition-3 d-flex align-items-center">
                ...
                <div className="category__content">
                    <h4 className="category__title"><Link href={genLinkUrlSub(subject.subject_name)}>{subject.subject_name}</Link></h4>
                    <p><Link href={genLinkUrlSub(subject.subject_name)}>{subject.tutor_count + ' Tutors Pan India'}</Link></p>
                </div>
            </div>
        </div></Link>
    )
