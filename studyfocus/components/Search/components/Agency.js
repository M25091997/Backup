...
export default function Agency() {
    ...
    const agencyData = useSelector(state => state.searchTutorAgency);
    let Pagination = () => {
        // if (!blogsData.blogs) return "";
        return <ul className="d-flex align-items-center">{Array.from(Array(parseInt(agencyData.tutors.length / 6)), (e, i) => {
        ...
        <div className="row grid">
            {
                ...
                //     agencyData.agency.slice(from, to).map(tutor =>
                //         <Card x={tutor} type={"agency"} />


                //     ) : <center><h3>No Agency available with this search data</h3></center>
                agencyData.loading ?
                    <>
                    ...
                    agencyData.agency && agencyData.agency.length ? agencyData.agency.slice(from, to).map(tutor =>
                        <Card x={tutor} type={"agency"} />
                    ) : <center><img width={'50%'} src="assets/img/logo/notfound.jpg" alt="logo" /></center>
            }

            <div className="row">
                <div className="col-xxl-12">
                    <div className="basic-pagination wow fadeInUp mt-30" data-wow-delay=".2s">
                        {agencyData.agency && agencyData.agency.length ?
                            <Pagination /> : ""
                        }
                    </div>
                </div>
